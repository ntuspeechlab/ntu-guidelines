...
---
## Interactive mode
I recommend that you use interactive mode of slurm to get resource allocation and run it locally while the cluster is drained frequently

```
srun -w node08 -N 1 -c 2 --gres=gpu:1 --pty bash --
```
 
And then you would be at node08 with a gpu allocated for you through slurm. There would be a `bash` task if you check squeue.
